...
struct BGRImage {
	...
	// Calculate Histogram in serial / sequence
	std::vector< std::pair<BGRColor, int> > *calculateHistogram() {
		// Create histogram array
		int *histogramArray = (int *) malloc (BGRColor::TOTAL_POSSIBLE_COLORS * sizeof(int));
		long long i;
		for(int i = 0; i < BGRColor::TOTAL_POSSIBLE_COLORS; i++) { histogramArray[i] = 0; }
		// Add colors to histogram
		long long _size = nRows * nCols;
		for(i = 0; i < _size; i++) { histogramArray[getColor(i).getValue()]++; }
	...
	// Calculate Block Histogram in serial / sequence
	std::vector< std::pair<BGRColor, int> > *calculateBlockHistogram(int b_size, int _block) {
		// Check block size
		...
		// i = row, j = col
		for(i = 0; i < b_size; i++) { 
			for(j = 0; j < b_size; j++) { histogramArray[getColor(getPixel(b_size, _block, i, j)).getValue()]++; }
		}
		// Populate the return value
		std::vector< std::pair<BGRColor, int> > *histogram = new std::vector< std::pair<BGRColor, int> >();
		for(i = 0; i < BGRColor::TOTAL_POSSIBLE_COLORS; i++) 
...
int main(int argc, char *argv[]) {
	...
	// Calculate Histogram serialize
	// Histogram --> calculateHistogram()
	// Block Histogram --> calculateHistogram(b_size, _block)
	std::vector< std::pair<BGRColor, int> > *serialHistogram = image->calculateHistogram();
	//std::vector< std::pair<BGRColor, int> > *serialHistogram = image->calculateBlockHistogram(8, 0);
	// Get end time
	...
	writeToFile(*serialHistogram, "histogram-serial");
	// Delete vector
	delete serialHistogram;
	// Delete image
	delete image;
